...
class bidSPOT:
    ...
    def plot(self,run_results, with_alarm = True, label=None, show=False, save=None):
        """
        Plot the results given by the run
        ...
        ts_fig, = plt.plot(x,self.data,color=air_force_blue, label='value')
        fig = [ts_fig]
        
        if 'upper_thresholds' in K:
            thup = run_results['upper_thresholds']
            uth_fig, = plt.plot(x,thup,color=deep_saffron,lw=1,ls='dashed', label='upper')
            fig.append(uth_fig)
            
        if 'lower_thresholds' in K:
            thdown = run_results['lower_thresholds']
            lth_fig, = plt.plot(x,thdown,color=deep_saffron,lw=1,ls='dashed', label='lower')
            fig.append(lth_fig)
        
        if with_alarm and ('alarms' in K):
            alarm = run_results['alarms']
            if len(alarm)>0:
                al_fig = plt.scatter(alarm,self.data[alarm],color='tab:pink', marker='o', label='alarm')
                fig.append(al_fig)

        if label is not None:
            lb_fig = plt.scatter(np.arange(len(label))[label==1], self.data[label==1], color='tab:red', marker='x', label='true')
